...
class stat : public jubatus::server::common::mprpc::rpc_server {
 public:
  explicit stat(double timeout_sec) : rpc_server(timeout_sec) {
    ...
    rpc_server::add<bool(std::string, double)>("push", pfi::lang::bind(
        &Impl::push, impl, pfi::lang::_1, pfi::lang::_2));
    rpc_server::add<double(std::string)>("sum", pfi::lang::bind(&Impl::sum,
         impl, pfi::lang::_1));
    rpc_server::add<double(std::string)>("stddev", pfi::lang::bind(
        &Impl::stddev, impl, pfi::lang::_1));
    rpc_server::add<double(std::string)>("max", pfi::lang::bind(&Impl::max,
         impl, pfi::lang::_1));
    rpc_server::add<double(std::string)>("min", pfi::lang::bind(&Impl::min,
         impl, pfi::lang::_1));
    rpc_server::add<double(std::string)>("entropy", pfi::lang::bind(
        &Impl::entropy, impl, pfi::lang::_1));
    rpc_server::add<double(std::string, int32_t, double)>("moment",
