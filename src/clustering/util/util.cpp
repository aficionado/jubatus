...
double sum(const sfv_t& p) {
  double s = 0;
  for (sfv_t::const_iterator it = p.begin(); it != p.end(); ++it) {
    s += (*it).second;
  }
  return s;
}
...
void scalar_mul_and_add(const sfv_t& left, float s, sfv_t& right) {
  sfv_t::const_iterator l = left.begin();
  sfv_t::iterator       r = right.begin();
...
sfv_t add(const sfv_t& p1, const sfv_t& p2) {
  sfv_t ret;
  sfv_t::const_iterator it1 = p1.begin();
  sfv_t::const_iterator it2 = p2.begin();
...
pair<size_t, double> min_dist(const sfv_t& p, const vector<sfv_t>& P) {
  size_t idx = 0;
  double mindist = DBL_MAX;
  for (vector<sfv_t>::const_iterator it = P.begin(); it != P.end(); ++it) {
    double d = dist(p, *it);
    if (mindist > d) {
      idx = it - P.begin();
      mindist = d;
