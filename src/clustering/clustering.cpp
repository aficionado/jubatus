...
std::vector<sfv_t> clustering::get_k_center() const {
  return clustering_method_->get_k_center();
}

sfv_t clustering::get_nearest_center(const sfv_t& point) const {
  return clustering_method_->get_nearest_center(point);
}

wplist clustering::get_nearest_members(const sfv_t& point) const {
...
void clustering::register_mixables(
  ...
  ms->set_model(
    common::cshared_ptr<storage>(storage_.get()));
